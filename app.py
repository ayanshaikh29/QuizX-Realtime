...
def calculate_points(is_correct, time_taken, time_limit, rank=None):
    """
...
class Question(db.Model):
    ...
    order = db.Column(db.Integer, default=0)  # For future reordering
...
class PartialAnswer(db.Model):
    ...
    points = db.Column(db.Integer, default=0)  # ✅ REQUIRED
...
class Result(db.Model):
    ...
    time_taken = db.Column(db.Integer, nullable=False)
    total_points = db.Column(db.Integer, default=0)  # ✅ REQUIRED
    submitted_at = db.Column(db.DateTime, default=now_utc)
...
def add_question(quiz_id):
    ...
    if request.method == "POST":
        ...
        for i, qtext in enumerate(questions_data):
            ...
            q = Question(
                quiz_id=quiz_id,
                order=i + 1,  # Set order based on addition sequence
                question=qtext,
...
def admin_live_leaderboard(quiz_id):
    ...
    return render_template("admin_live_leaderboard.html", 
                           quiz=quiz, 
                           quiz_id=quiz_id, 
                           qindex='live', 
...
def join_by_code():
    ...
    if request.method == "POST":
        ...
        quiz = Quiz.query.filter_by(join_code=code, is_active=True, is_published=True).first()
        if quiz:
...
def attempt_quiz(quiz_id):
    ...
    if request.method == "POST":
        ...
        question_points = 0
        if is_correct:
            # Get current question for time limit
            time_limit = current_question.time_limit
            question_points = calculate_points(is_correct, time_taken, time_limit)
        ...
        if is_last:
            ...
            partials = PartialAnswer.query.filter_by(quiz_id=quiz_id, student=student_name).all()
            ...
            print(f"Score: {score}/{total}, Time: {total_time}s, Points: {total_points}")

            # Save final result for all (guests and logged-in)
            existing = Result.query.filter_by(quiz_id=quiz_id, student=student_name).first()
            if not existing:
                result = Result(
                    quiz_id=quiz_id,
                    student=student_name,
            ...
            socketio.emit("leaderboard_refresh", {"quiz_id": quiz_id}, room=str(quiz_id))
    ...
    return render_template("attempt_quiz.html", quiz=quiz, current_question=current_question, total_questions=total_questions, current_index=qindex, quiz_id=quiz_id)
...
def api_leaderboard(quiz_id):
    ...
    points_query = db.session.query(
        ...
        func.sum(db.case((PartialAnswer.is_correct == True, 1), else_=0)).label('correct_count'),
